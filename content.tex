...
As explained in \citet{li_dong_2016}, the pitching-rolling kinematics has the potential to serve as a better canonical model for the hydrodynamics of bio-inspired flapping propulsors.
The authors carried on a parametric study, using their own research code, to quantify the effects of the Reynolds number, Strouhal number, aspect ratio, and rolling/pitching phase difference on the wake topology and the propulsive performance of flapping wings.
...
While \citet{li_dong_2016} do not report explicitly how they computed the hydrodynamic power, we feel the need to elaborate on it in the context of a diffuse-interface immersed boundary.
...
\begin{figure}[!h]
  ...
  \caption{History of the thrust ($C_T$), lift ($C_L$), and spanwise ($C_Z$) coefficients during the fourth and fifth flapping cycles of a circular plate ($AR = 1.27$) with $St = 0.6$, $Re = 200$, and $\psi = 90^o$. We also report the digitized coefficients from Fig. 9 of \cite{li_dong_2016} and the instantaneous values of the instantaneous rolling and pitching angles (dashed blue line and dotted green line, respectively).}
...
Although our numerical values do not fully match those from \citet{li_dong_2016}, we observe the same trends and thus consider this replication attempt to be successful.
...
From there, we often use multiple post-processing scripts to generate the final data and figures reported and analyzed in the manuscript; it involves computing secondary data, such as the vorticity field, the aerodynamic power and forces.
If the code is not made available, readers cannot inspect what has been done to produce these data; there could bugs introduced in these post-processing steps.
If no code is available, we cannot for sure explain discrepancies observed between our replication and the original study.
As \citet{donoho_et_al_2008} once said: ``The only way we'd ever get to the bottom of such a discrepancy is if we both worked reproducibly and studied detailed differences between code and data.''

We made our best efforts to make our replication study reproducible.
Our computational application makes use of open-source tools.
...
Admittedly, not everyone has access to a HPC cluster with GPU nodes and with Singularity installed.
Without those features, it becomes difficult to fully reproduce our workflow.
...
Once the Zenodo repository downloaded, readers should be able to spin up a Docker container and run a Bash script to compute the secondary data and generate the figures.
